...
CONTENT_TYPES = {
    ...
    "facts": {
        ...
        "system_prompt": (
            ...
            "6. اربط المعلومات ببعض بانتقال طبيعي بسيط بدل ما تبقى جمل مفككة.\n\n"
            "رد بصيغة JSON فقط بدون أي نص إضافي وبدون علامات markdown:\n"
    ...
    "mystery": {
        ...
        "system_prompt": (
            ...
            "5. قسّم القصة لجمل قصيرة (8-15 كلمة) تصلح سطر ترجمة على الشاشة.\n\n"
            "رد بصيغة JSON فقط بدون أي نص إضافي وبدون علامات markdown:\n"
            '{"title": "عنوان جذاب قصير (3-6 كلمات)", "sentences": ["الجملة الأولى", "الجملة الثانية", "..."]}'
        ),
        "title_templates": [
...
def sentence_to_speech(text, out_path, voice, rate, pitch):
    asyncio.run(_tts_save(text, out_path, voice, rate, pitch))
...
def build_story_audio(sentences, voice, rate, pitch):
    ...
    for i, sentence in enumerate(sentences):
        ...
        sentence_to_speech(clean, f_path, voice, rate, pitch)
...
def draw_caption_image(sentence, width, height, font_path):
    ...
    if char_count < 60:
        f_size, w_wrap, y_space = 92, 34, 118
    elif char_count < 120:
        f_size, w_wrap, y_space = 76, 39, 98
    else:
        f_size, w_wrap, y_space = 62, 44, 82

    font = ImageFont.truetype(font_path, f_size)
    lines = safe_wrap(sentence, width=w_wrap)

    total_h = len(lines) * y_space
    y_off = max(height * 0.60, (height - total_h) / 2 + height * 0.18)

    max_line_w = 0
    for line in lines:
        bbox = d.textbbox((0, 0), line, font=font, direction="rtl", language="ar")
...
def build_shorts_video():
    ...
    final = mp.CompositeVideoClip([bg, gradient, title_clip] + text_clips, size=(WIDTH, HEIGHT)).set_audio(final_audio)
